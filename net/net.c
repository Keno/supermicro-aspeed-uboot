...
# define ARP_TIMEOUT		500000UL	/* Milliseconds before trying ARP again; 5ms */
#else
...
NetSetTimeout(ulong iv, thand_f * f)
{
	if (iv == 0) {
		...
		iv = iv * (CONFIG_SYS_HZ / 1000); //ASPEED's uboot environment uses microsecond as timer unit
		timeHandler = f;
